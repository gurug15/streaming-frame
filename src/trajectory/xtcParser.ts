...
export function getFrameFile(data: Uint8Array): XtcFile {
...
// ============ STREAM-BASED OFFSET READER ============

export async function getData(path: string): Promise<number[]> {
    ...
    return new Promise<number[]>((resolve, reject) => {
        ...
        stream.on('data', (chunk: Buffer) => {
            const data: Uint8Array = concatAll([restData, new Uint8Array(chunk.buffer, chunk.byteOffset, chunk.byteLength)]);
